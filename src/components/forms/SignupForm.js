...
import { fetchAgreements } from '../../actions/agreementActions';
...
class SignupForm extends React.Component {
  ...
  componentWillMount() {
    // here must make the API Calls for agreements.
    fetchAgreements().then(data => {
      let agreements = {};
      data.agreements.forEach(agreement => {
        agreements[agreement.name] = {};
        agreements[agreement.name].name = agreement.name;
        agreements[agreement.name].header = agreement.header;
        agreements[agreement.name].short_message = agreement.short_message;
        agreements[agreement.name].policy = agreement.policy;
      });
      this.setState({
  ...
  validate = data => {
    ...
    if (!Validator.isEmail(data.email_address))
      errors.email_address = 'Debe ser un correo válido';
    return errors;
  };
      ...
      <Form onSubmit={this.onSubmit} loading={isLoading}>
        ...
        <Form.Field error={!!errors.email_address_confirmation}>
          ...
          <input
            ...
            onChange={this.onChange}
          />
          {errors.email_address_confirmation && (
            <InlineError text={errors.email_address_confirmation} />
          )}
        </Form.Field>
